...
const TeamMemberCard = ({ member, index }) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -5 }}
      className="glass rounded-xl overflow-hidden shadow-dark-md hover:shadow-glow border border-gray-700 transition-all"
    >
      <div className="relative overflow-hidden group">
        <img
          src={getProfileImagePath(member.discordId)}
          alt={member.name}
          className="w-full h-64 object-cover transition-transform duration-500 group-hover:scale-110 object-center"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-dark/90 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end">
          <div className="p-4 w-full">
            <div className="flex justify-center space-x-4">
              <a href={member.social.discord} target="_blank" rel="noopener noreferrer" className="text-white hover:text-primary transition-colors hover:scale-110 transform inline-block">
      ...
      <div className="p-6">
        <h3 className="text-xl font-bold mb-1 text-gray-200 group-hover:text-white transition-colors">{member.name}</h3>
        <div className="inline-block bg-gradient-to-r from-primary to-secondary px-3 py-1 rounded-full text-white text-sm font-medium mb-3 shimmer">{member.role}</div>
  ...
  return (
    <section id="team" ref={sectionRef} className="py-20 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden opacity-20">
        <div className="absolute top-1/4 right-1/3 w-64 h-64 rounded-full bg-primary opacity-20 blur-3xl"></div>
        <div className="absolute bottom-1/3 left-1/4 w-80 h-80 rounded-full bg-accent opacity-10 blur-3xl animate-float"></div>
      </div>
